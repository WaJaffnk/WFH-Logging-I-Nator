...
const { LogDatabase } = require('../app/LoggingDbService'); // Adjust path as needed
...
const dbService = new LogDatabase(knex);
...
describe('LoggingDbService class; findAll tests', () => {
    it('should filter logs by log_level', async () => {
        const expected = testLogs.filter(l => l.log_level === 'ERROR');
        tracker.on('query', (query) => {
            query.response(expected);
        });
        const logs = await dbService.findAll({ log_level: 'ERROR' });
        expect(logs.every(l => l.logLevel === 'ERROR')).toBe(true);
        expect(logs.length).toBe(expected.length);
    });

    it('should filter logs by log_category', async () => {
        const expected = testLogs.filter(l => l.log_category === 'api');
        tracker.on('query', (query) => {
            query.response(expected);
        });
        const logs = await dbService.findAll({ log_category: 'api' });
        expect(logs.every(l => l.logCategory === 'api')).toBe(true);
        expect(logs.length).toBe(expected.length);
    });

    it('should filter logs by publishing_service_name', async () => {
        const expected = testLogs.filter(l => l.publishing_service_name === 'workhorse-api');
        tracker.on('query', (query) => {
            query.response(expected);
        });
        const logs = await dbService.findAll({ publishing_service_name: 'workhorse-api' });
        expect(logs.every(l => l.publishingServiceName === 'workhorse-api')).toBe(true);
        expect(logs.length).toBe(expected.length);
    });

    it('should filter logs by consuming_service_name', async () => {
        const expected = testLogs.filter(l => l.consuming_service_name === 'rabbitmq');
        tracker.on('query', (query) => {
            query.response(expected);
        });
        const logs = await dbService.findAll({ consuming_service_name: 'rabbitmq' });
        expect(logs.every(l => l.consumingServiceName === 'rabbitmq')).toBe(true);
        expect(logs.length).toBe(expected.length);
    });

    it('should filter logs by message substring', async () => {
        const expected = testLogs.filter(l => l.message.includes('API request'));
        tracker.on('query', (query) => {
            query.response(expected);
        });
        const logs = await dbService.findAll({ message: 'API request' });
        expect(logs.every(l => l.message.includes('API request'))).toBe(true);
        expect(logs.length).toBe(expected.length);
    });
});
